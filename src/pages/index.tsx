...
export default function Home() {
  ...
  return (
        <div className="fixed top-0 right-0 left-0 bottom-0 gradient bg-opacity-50 flex" onClick={()=>setOpen(!open)}>
            <div className={`flex flex-col flex-shrink-0 items-stretch w-full gradient bg-opacity-80 rounded-r-lg sm:rounded-r-2xl shadow-xl overflow-hidden ${open ? 'max-w-screen-sm sm:max-w-[20rem]' : 'max-w-[64px]'} transition-all`}>
                <div className={`flex items-center ${open ? 'px-4' : 'px-2'} py-2 bg-black bg-opacity-5 dark:bg-opacity-25 shadow-lg transition-all`}>
                    ...
                    <div className={`mr-auto whitespace-nowrap ${open ? 'opacity-100' : 'opacity-0'} transition-opacity`}>
                        { selfData ?
                            <>
                ...
                <div className={`grid gap-x-2 gap-y-2 items-center py-2 ${open ? `px-4` : `px-2`} transition-all`} style={{gridTemplateColumns: open ? `min-content max-content` : `min-content 0`}}>
                    { true ?
                        <>
                            {[12, 16, 9, 10, 15, 9, 12, 15, 17, 10, 8].map((x, i) =>
                                ...
                                <div className="text-lg font-bold text-clip">1withspaghetti</div>
                        </>
                    :
                        <>
            ...
            <div className="flex flex-col w-full relative pt-2 pr-2 min-w-[calc(100vw-64px)] sm:min-w-0">
